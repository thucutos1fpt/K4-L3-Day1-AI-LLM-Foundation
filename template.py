...
def format_comparison_table(results: list[dict]) -> str:
    ...
    rows.extend(
        "| {prompt} | {gpt4o} | {mini} | {gpt4o_latency:.2f}s | {mini_latency:.2f}s |".format(
            prompt=shorten(result["prompt"]),
            gpt4o=shorten(result["gpt4o_response"]),
            mini=shorten(result["mini_response"]),
